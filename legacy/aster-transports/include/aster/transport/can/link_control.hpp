...
class CanLinkControlPlane {
 public:
  CanLinkControlPlane(CanLinkControlConfig config, CanFrameWriter physical_writer,
    ...
    std::size_t written{};
    valid_ = ConfigValid() &&
             HandshakeCodec::Encode(config_.local, handshake_payload_, written) ==
                 Status::kOk &&
  ...
  Status Poll(std::uint64_t now_ns,
    ...
    if (handshake_sender_.state() == ReliableSenderState::kWaitingForAck) {
      ...
      if (retry_status == Status::kOk) {
        ...
        if (const auto status = PumpHandshake(caller); status != Status::kOk) {
          return status;
        }
      } else if (retry_status != Status::kUnavailable &&
                 retry_status != Status::kTimeout) {
        return retry_status;
      }
    }
    ...
    if (!handshake_scheduled_ || now_ns >= next_handshake_ns_) {
      if (!SenderBusy(handshake_sender_.state())) {
        const auto status = handshake_sender_.Begin(
            kHandshakeRouteId, CanPriority::kCritical, handshake_sequence_,
            handshake_payload_, now_ns, config_.retry_timeout_ns,
            config_.maximum_retries);
        if (status != Status::kOk) return status;
        handshake_sequence_ =
            static_cast<std::uint8_t>((handshake_sequence_ + 1U) & 0x0fU);
        if (const auto pump_status = PumpHandshake(caller);
            pump_status != Status::kOk) {
          return pump_status;
        }
    ...
    if (!heartbeat_scheduled_ || now_ns >= next_heartbeat_ns_) {
      ...
      if (const auto status = physical_writer_.Send(frame, caller);
          status != Status::kOk) {
        return status;
      }
    ...
    if (config_.time_authority &&
        (!time_sync_scheduled_ || now_ns >= next_time_sync_ns_)) {
      CanFrame frame;
      const TimeSync sync{
          time_sync_sequence_++,
  ...
  Status Accept(const CanFrame& frame, std::uint64_t receive_local_time_ns,
    ...
    switch (id->route_id) {
      case kHandshakeRouteId:
        status = AcceptHandshake(frame, caller);
        break;
  ...
  bool compatible() const noexcept {
    return handshake_received_ &&
           compatibility_ == Compatibility::kCompatible;
  }
  ...
  std::uint64_t ToNetworkTime(std::uint64_t local_time_ns) const noexcept {
    return config_.time_authority ? local_time_ns
                                  : time_sync_.ToNetworkTime(local_time_ns);
  }
 ...
 private:
  ...
  bool ConfigValid() const noexcept {
    return config_.local.protocol_version != 0 && config_.local.node_id != 0 &&
           config_.peer_node_id != 0 &&
           config_.peer_node_id != config_.local.node_id &&
           config_.handshake_period_ns != 0 &&
           config_.heartbeat_period_ns != 0 &&
           config_.heartbeat_timeout_ns > config_.heartbeat_period_ns &&
           config_.time_sync_period_ns != 0 && config_.recovery_samples != 0 &&
           config_.retry_timeout_ns != 0 && physical_writer_.write != nullptr &&
           local_clock_.read != nullptr;
  }
  ...
  Status AcceptHandshake(
    ...
    const auto status =
        handshake_receiver_.Accept(frame, message, acknowledgement);
    ...
    return compatibility_ == Compatibility::kCompatible
               ? Status::kOk
               : Status::kTypeMismatch;
  }

  Status AcceptHeartbeat(const CanFrame& frame,
                         std::uint64_t receive_local_time_ns) noexcept {
    Heartbeat heartbeat;
  ...
  Status AcceptTimeSync(const CanFrame& frame,
                        std::uint64_t receive_local_time_ns) noexcept {
  ...
  void RefreshApplicationGate() noexcept {
    application_enabled_ = compatible() &&
                           heartbeat_.state() == LinkState::kOnline &&
                           time_synchronized();
  }
