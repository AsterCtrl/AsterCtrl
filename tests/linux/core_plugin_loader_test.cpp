...
int main() {
  ...
  const void* table{};
  assert(IsOk(loader.QueryInterface("test.backend", 1U, sizeof(TestBackend), table)));
  const auto& backend = *static_cast<const TestBackend*>(table);
  assert(backend.interface_version == 1U);
  assert(backend.struct_size == sizeof(TestBackend));
  assert(backend.read_value(backend.context) == 42);

  assert(loader.QueryInterface("missing.backend", 1U, sizeof(TestBackend), table) ==
         Status::kNotFound);
  assert(loader.QueryInterface("test.backend", 2U, sizeof(TestBackend), table) ==
         Status::kVersionMismatch);
  assert(loader.QueryInterface("test.backend", 1U, sizeof(TestBackend) + 1U, table) ==
         Status::kVersionMismatch);
  loader.Close();
  assert(loader.QueryInterface("test.backend", 1U, sizeof(TestBackend), table) ==
