...
from aster_tools.hardware_codegen import hardware_blockers, render_node_hardware
from aster_tools.firmware_codegen import (
    firmware_blockers,
...
_ASTER_CAN_CONTROL_IDS = range(1, 8)
...
def _allocate_route_ids(
    ...
    for link_name, link_reservations in reservations.items():
        for arbitration_id in _ASTER_CAN_CONTROL_IDS:
...
def _node_header(
    ...
    namespace = f"aster::generated::{node_name}"
    route_entries = []
    for route in routes:
...
// Generated by asterctl. Do not edit.
namespace {namespace} {{
...
struct CanLinkWriter {
  ...
  aster::transport::can::CanFrameWriter writer;
};
...
def _node_ports(
    ...
    constructor_parameters = [
        "      aster::runtime::SteadyClock& clock",
        "      std::span<const CanLinkWriter> writers",
        *[
            f"      aster::runtime::Executor& rpc_{index}_executor"
    ...
    for index, link_name in enumerate(links):
        ...
        control_initializers.append(
            f"        control_{index}_(\n"
            "            aster::transport::can::CanLinkControlConfig{\n"
            "                .local = aster::transport::can::Handshake{\n"
        ...
        if rpc["local_server"]:
            ...
            if rpc["remote_server"]:
                ...
                bridge_initializers.append(
                    ...
                    f"aster::transport::can::CanPriority::{priority}, "
    ...
    configure_lines = [
        "  aster::runtime::Status Configure() noexcept {",
    ...
    configure_lines.extend(
        [
            ...
            "  }",
            "",
            "  aster::runtime::Status Seal() noexcept {",
            "    using aster::runtime::IsOk;",
            "    using aster::runtime::Status;",
    ...
    configure_lines.extend(
        [
            ...
            "  }",
            "",
            "  aster::runtime::PortResolver& resolver() noexcept { return ports_; }",
            "",
    ...
    configure_lines.extend(
        [
            "  }",
            "",
            "  aster::transport::can::CanFrameReceiver CanReceiver(",
    ...
    configure_lines.extend(
        [
            ...
            "    return static_cast<aster::runtime::SteadyClock*>(state)->NowNs();",
            "  }",
            "",
            "  static aster::transport::can::CanFrameWriter FindWriter(",
            "      std::span<const CanLinkWriter> writers,",
            "      std::string_view link) noexcept {",
    ...
    for link_index, link in enumerate(links):
        dispatch_lines.extend(
            [
                f"  static aster::runtime::Status Receive{link_index}(",
                ...
                "  }",
                "",
                f"  aster::runtime::Status Dispatch{link_index}(",
    ...
    for index, _ in enumerate(links):
        field_lines.append(
            f"  aster::transport::can::CanFrameWriter writer_{index}_;"
        )
    for index, _ in enumerate(links):
        field_lines.append(
            f"  aster::transport::can::CanLinkControlPlane control_{index}_;"
        )
    for index, topic in enumerate(topics):
        ...
        field_lines.extend(
            [
                f"  aster::runtime::StaticTopicChannel<{cpp_type}, ",
                f"      {topic['sink_capacity']}, {topic['depth']}> topic_{index}_{{",
                f"          {_cpp_string(topic['name'])}, aster::runtime::DeliveryPolicy::{delivery}}};",
            ]
        )
        if topic["outgoing"]:
            route = topic["route"]
            ...
            field_lines.extend(
                [
                    f"  aster::transport::can::FastTopicEgress<{cpp_type}> egress_{index}_{{",
                    f"      {topic['route_id']}, aster::transport::can::CanPriority::{priority},",
        ...
        if topic["incoming"]:
            ...
            field_lines.extend(
                [
                    f"  aster::transport::can::FastTopicIngress<{cpp_type}> ingress_{index}_{{",
                    f"      {topic['route_id']}, topic_{index}_.publisher(),",
                    "      aster::transport::can::FreshnessConfig{",
                    f"          {deadline_ns}ULL, {max_age_ns}ULL,",
                    f"          aster::transport::can::RearmPolicy::{rearm}}}}};",
                ]
            )
    for index, rpc in enumerate(rpcs):
        ...
        if rpc["local_server"]:
            ...
            field_lines.append(
                f"  aster::runtime::{runtime_type}<{cpp_type}, "
            ...
            field_lines.append(
                f"  aster::transport::can::{transport_type}<{cpp_type}> rpc_{index}_;"
            )
    for index, rpc in enumerate(rpcs):
        ...
        field_lines.append(
            f"  aster::transport::can::{transport_type}<{cpp_type}> "
            f"rpc_bridge_{index}_;"
        )
    field_lines.extend(
        [
            f"  aster::runtime::StaticPortRegistry<{max(1, registry_entries)}> ports_;",
...
def _parameter_declaration(
    ...
    return (
        f"  aster::runtime::Parameter<{cpp_type}> "
        f"parameter_{instance_index}_{parameter_index}_{{\n"
        f"      aster::runtime::ParameterDescriptor<{cpp_type}>{{"
        f"{_cpp_string(parameter.name)}, {_cpp_string(parameter.unit)}, "
        f"{_cpp_parameter_value(parameter.type_name, parameter.value)}, "
        f"{_cpp_parameter_value(parameter.type_name, parameter.minimum)}, "
        f"{_cpp_parameter_value(parameter.type_name, parameter.maximum)}, "
        f"aster::runtime::ParameterMutability::k{mutability}, "
        f"aster::runtime::ParameterPersistence::k{persistence}}}}};"
    )
...
def _node_composition(
    ...
    public_lines = [
        ...
        "      aster::runtime::SteadyClock& clock,",
        "      aster::runtime::LogSink* log = nullptr,",
        "      aster::runtime::DiagnosticSink* diagnostics = nullptr) noexcept",
    ...
    public_lines.extend(
        [
            "",
            "  aster::runtime::Status Configure(",
            "      aster::runtime::HardwareResolver* hardware = nullptr) noexcept {",
            "    using aster::runtime::IsOk;",
            ...
            "",
            "  aster::runtime::Status Configure(",
            "      aster::runtime::PortResolver& ports,",
            "      aster::runtime::HardwareResolver* hardware = nullptr) noexcept {",
            "    using aster::runtime::IsOk;",
            "    using aster::runtime::Status;",
            "    if (configuration_attempted_) return Status::kInvalidState;",
    ...
    public_lines.extend(
        [
            ...
            "    return ports_.Seal();",
            "  }",
            "  aster::runtime::Status Start() noexcept {",
            ...
            "    if (runtime_.state() == aster::runtime::RuntimeState::kRunning) {",
            "      runtime_status = runtime_.Poll(now_ns, caller);",
            "    } else if (runtime_.state() !=",
            "               aster::runtime::RuntimeState::kInitialized) {",
            ...
            "  aster::runtime::Executor* FindExecutor(std::string_view name) noexcept {",
        ]
    )
    ...
    public_lines.extend(
        [
            "    return nullptr;",
            "  }",
            "  aster::runtime::Status RunExecutor(std::size_t index) noexcept {",
            "    switch (index) {",
        ]
    )
    ...
    public_lines.extend(
        [
            "      default: return aster::runtime::Status::kInvalidArgument;",
            "    }",
            "  }",
            "  aster::runtime::Status DrainExecutors(",
    ...
    public_lines.extend(
        [
            ...
            "  }",
            "  aster::runtime::Runtime& runtime() noexcept { return runtime_; }",
            "",
            " private:",
        ]
    )
    ...
    for index, (_, executor, full_name) in enumerate(executor_specs):
        private_lines.append(
            f"  aster::runtime::CooperativeExecutor<{executor['queue_depth']}> "
            f"executor_{index}_{{{_cpp_string(full_name)}, {executor['priority']}}};"
        )
    if periodic_specs:
        private_lines.append(
            f"  aster::runtime::StaticPeriodicScheduler<{len(periodic_specs)}> "
            'scheduler_{"periodic"};'
        )
    ...
    for instance_index, instance in enumerate(instances):
        if instance.config.get("ports"):
            ...
            private_lines.extend(
                [
                    f"  inline static constexpr std::array<aster::runtime::NameMapping, {len(instance.config['ports'])}> port_mappings_{instance_index}_{{{{",
                    mappings,
                    "  }};",
                    f"  aster::runtime::MappedPortResolver ports_{instance_index}_{{port_mappings_{instance_index}_}};",
                ]
            )
        if instance.config.get("hardware"):
            mappings = ",\n".join(
            ...
            private_lines.extend(
                [
                    f"  inline static constexpr std::array<aster::runtime::NameMapping, {len(instance.config['hardware'])}> hardware_mappings_{instance_index}_{{{{",
                    mappings,
                    "  }};",
                    f"  aster::runtime::MappedHardwareResolver hardware_{instance_index}_{{hardware_mappings_{instance_index}_}};",
                ]
            )
        ...
        if instance.parameters:
            private_lines.append(
                f"  aster::runtime::StaticParameterRegistry<{len(instance.parameters)}> "
                f"parameters_{instance_index}_;"
            )
    for index, _ in enumerate(instances):
        private_lines.append(f"  aster::runtime::ModuleContext context_{index}_;")
    executor_slot_entries = ", ".join(
        f"aster::runtime::ExecutorSlot{{&executor_{index}_}}"
        for index, _ in enumerate(executor_specs)
    )
    module_slot_entries = ", ".join(
        f"aster::runtime::ModuleSlot{{&module_{index}_, &context_{index}_}}"
        for index, _ in enumerate(instances)
    )
    private_lines.extend(
        [
            f"  std::array<aster::runtime::ExecutorSlot, {len(executor_specs)}> executor_slots_{{{{{executor_slot_entries}}}}};",
            f"  std::array<aster::runtime::ModuleSlot, {len(instances)}> module_slots_{{{{{module_slot_entries}}}}};",
            "  aster::runtime::Runtime runtime_;",
            "  bool configuration_attempted_{};",
            "  bool configured_{};",
