...
def _render_one_encode(
    ...
    if symbol in PRIMITIVE_SIZES or symbol in model.enums:
        return [
            f"{indent}if (const auto status = ::robot_msgs::detail::WriteScalar({expression}, output, offset);",
...
def _render_one_decode(
    ...
    if symbol in PRIMITIVE_SIZES or symbol in model.enums:
        return [
            f"{indent}if (const auto status = ::robot_msgs::detail::ReadScalar(input, offset, {expression});",
            f"{indent}    !IsOk(status)) {{",
            f"{indent}  return status;",
            f"{indent}}}",
        ]
...
DETAIL_HELPERS = r"""namespace robot_msgs::detail {
