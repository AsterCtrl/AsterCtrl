...
struct ModuleSlot {
  ...
  constexpr ModuleSlot() noexcept = default;
  ModuleSlot(ModuleBase* value, CoreRef core_ref, std::string_view name) noexcept
      : module(value == nullptr ? ModuleRef{} : ModuleRef(*value)),
        core(core_ref),
        instance_name(name) {}
  constexpr ModuleSlot(ModuleRef value, CoreRef core_ref, std::string_view name) noexcept
      : module(value), core(core_ref), instance_name(name) {}
};
