...
#include "aster_runtime/transport/transport.hpp"
...
class ChannelTransportModule final : public ModuleBase {
 public:
  ...
  Status Start() noexcept override {
    ...
    if (IsOk(status)) status = Schedule({name_, ExecutionKind::kThread, now_ns}, now_ns);
    if (!IsOk(status)) {
      running_ = false;
      transport_.Stop();
 ...
 private:
  ...
  void PollOnce(const ExecutionContext& caller) noexcept {
    ...
    std::uint64_t now_ns{};
    auto schedule_status = clock_.NowNs(now_ns);
    if (IsOk(schedule_status)) schedule_status = Schedule(caller, now_ns);
    if (!IsOk(schedule_status)) {
  ...
  Status Schedule(const ExecutionContext& caller, std::uint64_t now_ns) noexcept {
    return executor_.TryPostAt(SaturatingAdd(now_ns, poll_interval_ns_), WorkItem::Bind<Poll>(this),
                               caller);
  }
