...
class CanRpcClient final : public RpcBackend {
 public:
  ...
  Status CallAsync(const ServiceDescriptor& descriptor, std::span<const std::byte> request,
    ...
    std::uint64_t now_ns{};
    const auto clock_status = clock_.NowNs(now_ns);
    if (!IsOk(clock_status)) return Reject(clock_status);
  ...
  Status Accept(const CanFrame& frame, std::uint64_t, const ExecutionContext& caller) noexcept {
    ...
    completion(completion_state, ToAbiStatus(response_status),
               reinterpret_cast<const std::uint8_t*>(response.data()), response.size(), &abi_info,
               &abi_context);
    return Status::kOk;
  }
  ...
  void ResetPeer(const ExecutionContext& caller) noexcept {
    ...
    const auto abi_info = ToAbiRpcCallInfo(info);
    const auto abi_context = ToAbiExecutionContext(caller);
    completion(completion_state, ASTER_STATUS_UNAVAILABLE, nullptr, 0, &abi_info, &abi_context);
  }
 ...
 private:
  ...
  void FinishTimeout(const ExecutionContext& caller) noexcept {
    ...
    const auto abi_info = ToAbiRpcCallInfo(info);
    const auto abi_context = ToAbiExecutionContext(caller);
    completion(completion_state, ASTER_STATUS_TIMEOUT, nullptr, 0, &abi_info, &abi_context);
  }
...
class CanRpcServer {
 ...
 private:
  ...
  static void Complete(void* state, aster_status_t status, const std::uint8_t* response,
                       std::size_t response_size, const aster_rpc_call_info_t*,
                       const aster_execution_context_t* context) noexcept {
    if (context == nullptr || (response == nullptr && response_size != 0)) {
      return;
    }
    ...
    if (!IsOk(self.SendResponse(FromAbiStatus(status),
                                {reinterpret_cast<const std::byte*>(response), response_size},
                                FromAbiExecutionContext(*context)))) {
  ...
  Status SendResponse(Status status, std::span<const std::byte> response,
    ...
    std::uint64_t now_ns{};
    const auto clock_status = clock_.NowNs(now_ns);
    if (!IsOk(clock_status)) return clock_status;
    const auto begin_status =
        response_sender_.Begin(route_id_, priority_, active_sequence_, payload, now_ns,
