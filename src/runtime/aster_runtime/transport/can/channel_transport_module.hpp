...
class CanChannelTransportModule final : public ModuleBase {
 public:
  ...
  Status Start() noexcept override {
    ...
    running_ = true;
    std::uint64_t now_ns{};
    status = clock_.NowNs(now_ns);
    if (IsOk(status))
      status = executor_.TryPostAt(now_ns, WorkItem::Bind<Poll>(this),
                                   {name_, ExecutionKind::kThread, now_ns});
    if (!IsOk(status)) {
      running_ = false;
 ...
 private:
  ...
  static Status ReadClock(void* state, std::uint64_t& output) noexcept {
    return static_cast<CanChannelTransportModule*>(state)->clock_.NowNs(output);
  }

  static Status ReadNetworkClock(void* state, std::uint64_t& output) noexcept {
    auto& self = *static_cast<CanChannelTransportModule*>(state);
    std::uint64_t now{};
    const auto status = self.clock_.NowNs(now);
    if (IsOk(status)) output = self.control_.ToNetworkTime(now);
    return status;
  }
  ...
  void PollOnce(const ExecutionContext& caller) noexcept {
    ...
    const auto status = executor_.TryPostAt(SaturatingAdd(now_ns, poll_interval_ns_),
                                            WorkItem::Bind<Poll>(this), caller);
    if (!IsOk(status)) {
