...
## Current configuration-driven Runtime

The v1alpha3 Host launcher supports only the Local Channel and RPC backends.
Their queues are sized by runtime.yaml, not an MCU-wide compile-time limit.
Callbacks run on the receiving instance's executor and own their encoded data;
full queues return backpressure. See [runtime-v3](runtime-v3.md) for broadcast,
timeout and shutdown semantics.

CAN/SocketCAN and USB implementations below are retained adapters and v1alpha2
...
UDP, Zenoh, ROS 2, AimRT, gRPC and MQTT are deferred. A Transport interface or
CorePlugin query entry is not a working plugin system: production plugin
lifecycle and backend registration are still missing. Do not configure these
names in the new launcher expecting an installed backend.

The old handshake uses the full Deployment Hash. Separating the communication
contract identity from business configuration and artifact integrity remains
an [open implementation item](../development/convergence-audit.md).
