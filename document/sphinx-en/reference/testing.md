...
- Transport tests cover Local dispatch, CAN fragmentation/loss/reorder/restart,
  ...
  Runtime shutdown.
- The retained Zephyr test suite is intended to execute the portable pub/sub Module source in `native_sim` and
  ...
  jobs have not been executed for the current convergence work on this macOS
  host; the new v1alpha3 static-deployment path is also not implemented yet.
