...
class HandshakeCodec {
 public:
  ...
  static Status Encode(const Handshake& handshake, std::span<std::byte> output,
    ...
    written = 0;
    if (output.size() < kEncodedSize || handshake.protocol_version == 0 || handshake.node_id == 0) {
      return output.size() < kEncodedSize ? Status::kCapacityExceeded : Status::kInvalidArgument;
    }
    ...
    std::copy(handshake.deployment_hash.begin(), handshake.deployment_hash.end(),
              output.begin() + 2);
    std::copy(handshake.schema_hash.begin(), handshake.schema_hash.end(), output.begin() + 18);
    written = kEncodedSize;
    return Status::kOk;
  }

  static Status Decode(std::span<const std::byte> input, Handshake& handshake) noexcept {
...
class ControlFrameCodec {
 public:
  static Status EncodeHeartbeat(const Heartbeat& heartbeat, CanFrame& frame) noexcept {
    if (heartbeat.node_id == 0) {
      return Status::kInvalidArgument;
    }
    frame = {};
    if (const auto status = CanArbitrationId::Encode(CanPriority::kCritical, kHeartbeatRouteId,
  ...
  static Status DecodeHeartbeat(const CanFrame& frame, Heartbeat& heartbeat) noexcept {
    const auto id = CanArbitrationId::Decode(frame.arbitration_id);
    if (!id.has_value() || id->priority != CanPriority::kCritical ||
        id->route_id != kHeartbeatRouteId || frame.size != 8 || frame.data[0] != std::byte{0xc0}) {
      return Status::kInvalidArgument;
    }
  ...
  static Status EncodeTimeSync(const TimeSync& sync, CanFrame& frame) noexcept {
    ...
    frame = {};
    if (const auto status = CanArbitrationId::Encode(CanPriority::kCritical, kTimeSyncRouteId,
                                                     frame.arbitration_id);
        status != Status::kOk) {
      return status;
    }
    ...
    for (std::size_t index = 0; index < 6; ++index) {
      frame.data[index + 2] = static_cast<std::byte>(sync.master_time_us >> (index * 8U));
    }
    return Status::kOk;
  }
  ...
  static Status DecodeTimeSync(const CanFrame& frame, TimeSync& sync) noexcept {
    const auto id = CanArbitrationId::Decode(frame.arbitration_id);
    if (!id.has_value() || id->priority != CanPriority::kCritical ||
        id->route_id != kTimeSyncRouteId || frame.size != 8 || frame.data[0] != std::byte{0xc1}) {
      return Status::kInvalidArgument;
    }
    ...
    for (std::size_t index = 0; index < 6; ++index) {
      sync.master_time_us |=
          static_cast<std::uint64_t>(std::to_integer<std::uint8_t>(frame.data[index + 2]))
          << (index * 8U);
    }
    return Status::kOk;
  }
 ...
 private:
  static void WriteU32(std::uint32_t value, std::array<std::byte, 8>::iterator output) noexcept {
    for (std::size_t index = 0; index < 4; ++index) {
      output[static_cast<std::ptrdiff_t>(index)] = static_cast<std::byte>(value >> (index * 8U));
    }
  }

  static std::uint32_t ReadU32(std::array<std::byte, 8>::const_iterator input) noexcept {
    std::uint32_t value{};
    for (std::size_t index = 0; index < 4; ++index) {
      value |= static_cast<std::uint32_t>(
                   std::to_integer<std::uint8_t>(input[static_cast<std::ptrdiff_t>(index)]))
               << (index * 8U);
    }
    return value;
  }
...
class CompatibilityGuard {
 public:
  constexpr Compatibility Check(const Handshake& local, const Handshake& remote) const noexcept {
...
class HeartbeatMonitor {
 public:
  constexpr HeartbeatMonitor(std::uint64_t timeout_ns, std::uint8_t recovery_samples) noexcept
  ...
  LinkState Observe(std::uint8_t sequence, std::uint64_t now_ns) noexcept {
    ...
    state_ =
        consecutive_samples_ >= recovery_samples_ ? LinkState::kOnline : LinkState::kRecovering;
    return state_;
  }
...
class TimeSyncFollower {
 public:
  void Observe(std::uint64_t master_time_ns, std::uint64_t local_receive_time_ns) noexcept {
...
class FreshnessGuard {
 public:
  constexpr explicit FreshnessGuard(FreshnessConfig config) noexcept : config_(config) {}

  FreshnessState Observe(std::uint8_t sequence, std::uint64_t source_timestamp_ns,
                         std::uint64_t receive_timestamp_ns) noexcept {
    const auto age =
        source_timestamp_ns > receive_timestamp_ns ? 0 : receive_timestamp_ns - source_timestamp_ns;
    last_sequence_ = sequence;
    ...
    if (state_ == FreshnessState::kStale && config_.rearm == RearmPolicy::kExplicit &&
        !explicitly_rearmed_) {
      return state_;
    }
  ...
  FreshnessState Poll(std::uint64_t now_ns) noexcept {
    ...
    if (config_.deadline_ns != 0 && now_ns - last_receive_ns_ >= config_.deadline_ns) {
      state_ = FreshnessState::kStale;
    }
    return state_;
  }
