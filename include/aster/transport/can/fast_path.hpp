...
class FastCodec {
 public:
  static Status Encode(std::uint16_t route_id, CanPriority priority, std::uint8_t sequence,
                       std::span<const std::byte> payload, std::span<CanFrame> output,
    ...
    if (const auto status = CanArbitrationId::Encode(priority, route_id, arbitration_id);
        status != Status::kOk) {
      return status;
    }
    ...
    for (std::size_t index = 0; index < count; ++index) {
      ...
      frame.data[1] = static_cast<std::byte>(((static_cast<std::uint8_t>(count) - 1U) << 4U) |
...
class FastReassembler {
 public:
  ...
  Status Accept(const CanFrame& frame, ReassembledMessage& message) noexcept {
    ...
    if (kind == FrameKind::kFastSingle) {
      ...
      std::copy_n(frame.data.begin() + 1, static_cast<std::ptrdiff_t>(length), buffer_.begin());
 ...
 private:
  ...
  Status PrepareSequence(std::uint16_t route_id, std::uint8_t sequence,
    ...
    if (active_) {
      if (route_id != route_id_) {
        ...
        const auto active_distance =
            static_cast<std::uint8_t>((static_cast<std::uint8_t>(sequence - sequence_)) & 0x3fU);
    ...
    const auto completed_distance =
        static_cast<std::uint8_t>((static_cast<std::uint8_t>(sequence - last_sequence_)) & 0x3fU);
  ...
  void Complete(std::uint16_t route_id, std::uint8_t sequence, std::size_t size,
                ReassembledMessage& message) noexcept {
    if (has_last_sequence_ && last_route_id_ == route_id) {
    ...
    message = {route_id, sequence, std::span<const std::byte>(buffer_.data(), size)};
    ++stats_.completed;
  }
