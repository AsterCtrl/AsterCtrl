...
class CanArbitrationId {
 public:
  ...
  static constexpr std::optional<DecodedArbitrationId> Decode(std::uint16_t encoded) noexcept {
    if (encoded > 0x7ffU || (encoded & 0x1ffU) == 0) {
      return std::nullopt;
    }
    return DecodedArbitrationId{static_cast<CanPriority>((encoded >> 9U) & 0x3U),
                                static_cast<std::uint16_t>(encoded & 0x1ffU)};
  }
};

struct CanFrame {
  using WriteCompletion = void (*)(void*, std::uint32_t, Status) noexcept;
  ...
  void* write_state{};
  std::uint32_t write_token{};

  void CompleteWrite(Status status) const noexcept {
    if (write_completion != nullptr) {
      write_completion(write_state, write_token, status);
    }
  }
};
...
constexpr FrameKind GetFrameKind(std::byte header) noexcept {
  return static_cast<FrameKind>((std::to_integer<std::uint8_t>(header) >> 6U) & 0x3U);
}
